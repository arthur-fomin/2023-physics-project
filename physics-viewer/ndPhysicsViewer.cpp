...
ndInt32 ndPhysicsViewer::ButtonKey::UpdateTrigger (bool triggerValue)
{
	m_memory0 = m_memory1;
...
ndInt32 ndPhysicsViewer::ButtonKey::UpdatePushButton (bool triggerValue)
{
...
class ndPhysicsViewer::ndPhysicsViewer::ndDebugMesh
{
	public:
	ndDebugMesh()
		:m_flatShaded()
...
class ndPhysicsViewer::ndPhysicsViewer::ndDebugMeshCache : public ndTree<ndDebugMesh, const ndShape*>
{
};

// ImGui - standalone example application for Glfw + OpenGL 2, using fixed pipeline
// If you are new to ImGui, see examples/README.txt and documentation at the top of imgui.cpp.
ndPhysicsViewer::ndPhysicsViewer ()
	:m_mainFrame(nullptr)
	,m_defaultFont(0)
	,m_sky(nullptr)
	,m_world(nullptr)
	...
	,m_replayLogFile(nullptr)
{
	...
	//ndSharedPtr<ndPhysicsViewer> xxx(this);
	//ndPhysicsViewer* xxx1 = *xxx;
	//ndPhysicsViewer* xxx2 = *xxx;
}
...
#ifdef _DEBUG
void ndPhysicsViewer::OpenMessageCallback(GLenum source,
	GLenum type,
	GLuint id,
	GLenum severity,
	GLsizei length,
...
ndLabCamera* ndPhysicsViewer::GetCamera() const
{
	return m_cameraManager->GetCamera();
}

bool ndPhysicsViewer::GetKeyState(ndInt32 key) const
{
...
ndAnimationSequence* ndPhysicsViewer::GetAnimationSequence(const char* const fileName)
{
...
bool ndPhysicsViewer::IsShiftKeyDown () const
{
	const ImGuiIO& io = ImGui::GetIO();
...
bool ndPhysicsViewer::IsControlKeyDown () const
{
	const ImGuiIO& io = ImGui::GetIO();
...
bool ndPhysicsViewer::GetCaptured() const
{
	ImGuiIO& io = ImGui::GetIO();
	return io.WantCaptureMouse;
}

bool ndPhysicsViewer::GetMouseKeyState (ndInt32 button) const
{
	ImGuiIO& io = ImGui::GetIO();
	return io.MouseDown[button];
}

void ndPhysicsViewer::Set2DDisplayRenderFunction(ndSharedPtr<ndUIEntity>& demoGui)
{
	m_renderDemoGUI = demoGui;
}

void* ndPhysicsViewer::GetUpdateCameraContext() const
{
	return m_updateCameraContext;
}

void ndPhysicsViewer::SetUpdateCameraFunction(UpdateCameraCallback callback, void* const context)
{
	m_updateCamera = callback;
	m_updateCameraContext = context;
}

bool ndPhysicsViewer::JoystickDetected() const
{
	return glfwJoystickPresent(0) ? true : false;
}

void ndPhysicsViewer::GetJoystickAxis (ndFixSizeArray<ndFloat32, 8>& axisValues)
{
...
void ndPhysicsViewer::GetJoystickButtons(ndFixSizeArray<char, 32>& axisbuttons)
{
	ndAssert(JoystickDetected());
...
void ndPhysicsViewer::ResetTimer()
{
	dResetTimer();
	m_microsecunds = ndGetTimeInMicroseconds ();
}

void ndPhysicsViewer::AddEntity(ndBaseEntity* const ent)
{
...
void ndPhysicsViewer::RemoveEntity (ndBaseEntity* const ent)
{
	ndScopeSpinLock lock(m_addDeleteLock);
	ndAssert(ent->m_rootNode);
...
void ndPhysicsViewer::Cleanup ()
{
	...
	if (m_world) 
	{
		...
		for (ndBodyListView::ndNode* bodyNode = bodyList.GetFirst(); bodyNode; bodyNode = bodyNode->GetNext())
		{
			...
			ndBaseEntityNotify* const callback = (ndBaseEntityNotify*)body->GetNotifyCallback();
			if (callback)
			{
	...
	while (GetFirst())
	{
		ndBaseEntity* const ent = GetFirst()->GetInfo();
		RemoveEntity(ent);
		delete ent;
	}
	...
	m_cameraManager = new ndLabCameraManager(this);
...
void ndPhysicsViewer::ApplyMenuOptions()
{
	m_world->Sync();
...
void ndPhysicsViewer::ShowMainMenuBar()
{
...
void ndPhysicsViewer::LoadDemo(ndInt32 menu)
{
	char newTitle[256];
	Cleanup();
	...
	sprintf(newTitle, "Artur Fomin 3Body Rattleback Simulation" 
    // D_NEWTON_ENGINE_MAJOR_VERSION, D_NEWTON_ENGINE_MINOR_VERSION, m_demosSelection[menu].m_name
    );
...
void ndPhysicsViewer::ErrorCallback(ndInt32 error, const char* description)
{
	ndTrace (("Error %d: %s\n", error, description));
...
void ndPhysicsViewer::MouseButtonCallback(GLFWwindow*, ndInt32 button, ndInt32 action, ndInt32)
{
...
void ndPhysicsViewer::MouseScrollCallback(GLFWwindow* const, double, double y)
{
	ImGuiIO& io = ImGui::GetIO();
	io.MouseWheel += float (y);
}

void ndPhysicsViewer::CursorposCallback  (GLFWwindow* , double x, double y)
{
	ImGuiIO& io = ImGui::GetIO();
	io.MousePos = ImVec2((float)x, (float)y);
}

bool ndPhysicsViewer::GetMouseSpeed(ndFloat32& speedX, ndFloat32& speedY) const
{
...
bool ndPhysicsViewer::GetMousePosition (ndFloat32& posX, ndFloat32& posY) const
{
...
void ndPhysicsViewer::CharCallback(GLFWwindow*, ndUnsigned32 ch)
{
	ImGuiIO& io = ImGui::GetIO();
...
void ndPhysicsViewer::KeyCallback(GLFWwindow* const window, ndInt32 key, ndInt32, ndInt32 action, ndInt32 mods)
{
	ImGuiIO& io = ImGui::GetIO();
	if (action == GLFW_PRESS)
	...
	ndPhysicsViewer* const manager = (ndPhysicsViewer*)glfwGetWindowUserPointer(window);
...
void ndPhysicsViewer::ToggleProfiler()
{
	#ifdef D_PROFILER
		ndAssert(m_world);
...
void ndPhysicsViewer::BeginFrame()
{
...
ndInt32 ndPhysicsViewer::ParticleCount() const
{
	ndInt32 count = 0;
...
void ndPhysicsViewer::SetParticleUpdateMode() const
{
	const ndBodyList& particles = m_world->GetParticleList();
	for (ndBodyList::ndNode* node = particles.GetFirst(); node; node = node->GetNext())
	{
...
void ndPhysicsViewer::RenderStats()
{
	if (m_showStats) 
	{
		char text[1024];
...
void ndPhysicsViewer::CalculateFPS(ndFloat32 timestep)
{
	m_framesCount ++;
...
void ndPhysicsViewer::CreateSkyBox()
{
	if (!m_sky)
	{
...
void ndPhysicsViewer::PushTransparentMesh (const ndDemoMeshInterface* const mesh, const ndMatrix& modelMatrix)
{
...
//void ndPhysicsViewer::ImportPLYfile (const char* const fileName)
void ndPhysicsViewer::ImportPLYfile(const char* const)
{
	ndAssert(0);
	//m_collisionDisplayMode = 2;
...
ndInt32 ndPhysicsViewer::Print (const ndVector&, const char *fmt, ... ) const
{
	va_list argptr;
	char string[1024];
...
void ndPhysicsViewer::SetCameraMatrix (const ndQuaternion& rotation, const ndVector& position)
{
	m_cameraManager->SetCameraMatrix(rotation, position);
}

void ndPhysicsViewer::UpdatePhysics(ndFloat32 timestep)
{
	// update the physics
...
ndFloat32 ndPhysicsViewer::CalculateInteplationParam () const
{
...
void ndPhysicsViewer::RenderScene(ImDrawData* const draw_data)
{
	...
	ndPhysicsViewer* const window = (ndPhysicsViewer*)io.UserData;
...
void ndPhysicsViewer::DrawDebugShapes()
{
...
void ndPhysicsViewer::RenderScene()
{
	D_TRACKTIME();
	...
	{
		for (ndNode* node = ndList<ndBaseEntity*>::GetFirst(); node; node = node->GetNext()) 
		{
...
void ndPhysicsViewer::TestImGui()
{
	// Main loop
