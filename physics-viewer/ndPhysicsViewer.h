...
class ndLabCameraManager;
class ndWireFrameDebugMesh;
class ndFlatShadedDebugMesh;

class ndPhysicsViewer: public ndList <ndBaseEntity*>
{
	public:
	typedef void (*LaunchSDKDemoCallback) (ndPhysicsViewer* const scene);
	typedef void(*UpdateCameraCallback) (ndPhysicsViewer* const manager, void* const context, ndFloat32 timestep);
	...
	void AddEntity(ndBaseEntity* const ent);
	void RemoveEntity(ndBaseEntity* const ent);
	...
	ndLabCameraManager* GetCameraManager() const;
	...
	ndLabCameraManager* m_cameraManager;
...
inline ndPhysicsWorld* ndPhysicsViewer::GetWorld() const
{
	return m_world;
}

inline ndInt32 ndPhysicsViewer::GetWidth() const 
{ 
	ImGuiIO& io = ImGui::GetIO();
	return (ndInt32)(io.DisplaySize.x * io.DisplayFramebufferScale.x);
}

inline ndInt32 ndPhysicsViewer::GetHeight() const 
{ 
	ImGuiIO& io = ImGui::GetIO();
	return (ndInt32)(io.DisplaySize.y * io.DisplayFramebufferScale.y);
}

inline ndInt32 ndPhysicsViewer::GetDebugDisplay() const
{
	ndAssert (0);
	return 0;
}

inline void ndPhysicsViewer::SetDebugDisplay(ndInt32) const
{
	ndAssert (0);
}

inline const ndShaderCache& ndPhysicsViewer::GetShaderCache() const
{
	return m_shaderCache;
}

inline ndLabCameraManager* ndPhysicsViewer::GetCameraManager() const
{
	return m_cameraManager;
}

inline void ndPhysicsViewer::SetSelectedModel(ndModel* const model)
{
