	...
	,m_breakImpactSpeed(desc.m_breakImpactSpeed)
{
	...
	ndPhysicsViewer* const scene = manager->m_scene;
...
ndExplodeConvexShapeModel::ndExplodeConvexShapeModel(ndPhysicsViewer* const scene)
	:ndModel()
	,m_effectList()
	,m_pendingEffect()
	,m_scene(scene)
...
void ndExplodeConvexShapeModel::AddEffect(const ndEffect& effect, ndFloat32 mass, const ndMatrix& location)
{
	...
	ndBaseEntity* const entity = new ndBaseEntity(location, nullptr);
	entity->SetMesh(effect.m_visualMesh);
	m_scene->AddEntity(entity);
	
	m_scene->GetWorld()->AddBody(newEffect.m_body);
	
	newEffect.m_body->SetNotifyCallback(new ndBaseEntityNotify(m_scene, entity));
...
void ndExplodeConvexShapeModel::UpdateEffect(ndWorld* const world, ndEffect& effect)
{
	...
	ndPhysicsViewer* const scene = physicsWorld->GetManager();
	ndBaseEntityNotify* const notify = (ndBaseEntityNotify*)effect.m_body->GetNotifyCallback();
	ndBaseEntity* const visualEntity = (ndBaseEntity*)notify->GetUserData();

	ndMatrix matrix(visualEntity->GetCurrentMatrix());
	ndQuaternion rotation(matrix);

	ndBaseEntity* const debriRootEnt = effect.m_debrisRootEnt;
