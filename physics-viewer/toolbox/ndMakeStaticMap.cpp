...
ndBodyKinematic* BuildFloorBox(ndPhysicsViewer* const scene, const ndMatrix& matrix)
{
...
ndBodyKinematic* BuildStaticMesh(ndPhysicsViewer* const scene, const char* const meshName, bool optimized)
{
	...
	ndBodyKinematic* const body = new ndBodyDynamic();
	body->SetNotifyCallback(new ndBaseEntityNotify(scene, visualEntity));
	body->SetMatrix(matrix);
...
ndBodyKinematic* BuildPlayArena(ndPhysicsViewer* const scene)
{
	...
	ndBaseEntity* const entity = new ndBaseEntity(scene, meshEffectNode);
	scene->AddEntity(entity);
...
#if 1
	ndBaseEntity* const pivot0 = entity->Find("pivot1");
	ndBaseEntity* const pivot1 = entity->Find("pivot0");
...
ndBodyKinematic* BuildSplineTrack(ndPhysicsViewer* const scene, const char* const meshName, bool optimized)
{
	...
	ndMatrix matrix(ndGetIdentityMatrix());
	{
		...
		//ndBaseEntity* const splineEntity = new ndBaseEntity(matrix, nullptr);
