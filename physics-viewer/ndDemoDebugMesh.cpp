...
void ndFlatShadedDebugMesh::Render(ndPhysicsViewer* const scene, const ndMatrix& modelMatrix)
{
	if (m_shader)
	{
		glUseProgram(m_shader);
...
void ndWireFrameDebugMesh::Render(ndPhysicsViewer* const scene, const ndMatrix& modelMatrix)
{
	if (m_shader)
	{
		ndLabCamera* const camera = scene->GetCamera();
